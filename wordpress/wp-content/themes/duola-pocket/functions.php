...
function duola_pocket_get_desktop_image_id(): int
{
    $selected_image_id = (int) get_theme_mod('duola_desktop_image');
    ...
    foreach ($albums as $album) {
        $cover_id = duola_albums_get_cover_id((int) $album->ID);
        if ($cover_id) {
            return $cover_id;
        }

        $photos = duola_albums_get_photos((int) $album->ID);
        if ($photos) {
            return (int) $photos[0]['id'];
        }
    }
...
function duola_pocket_sanitize_image_position(string $value): string
{
    $positions = ['center center', 'center top', 'center bottom', 'left center', 'right center'];
    return in_array($value, $positions, true) ? $value : 'center center';
}

function duola_pocket_customize_register(WP_Customize_Manager $customize): void
{
    $customize->add_section('duola_desktop', [
        'title' => __('桌面封面', 'duola-pocket'),
        'priority' => 30,
    ]);
    $customize->add_setting('duola_desktop_image', [
        'sanitize_callback' => 'absint',
    ]);
    $customize->add_control(new WP_Customize_Media_Control($customize, 'duola_desktop_image', [
        'label' => __('桌面封面图', 'duola-pocket'),
        'section' => 'duola_desktop',
        'mime_type' => 'image',
    ]));
    $customize->add_setting('duola_desktop_caption', [
        'default' => '某年某月某天',
        'sanitize_callback' => 'sanitize_text_field',
    ]);
    $customize->add_control('duola_desktop_caption', [
        'label' => __('桌面主文字', 'duola-pocket'),
        'section' => 'duola_desktop',
        'type' => 'text',
    ]);
    $customize->add_setting('duola_desktop_note', [
        'default' => 'Stay alive!',
        'sanitize_callback' => 'sanitize_text_field',
    ]);
    $customize->add_control('duola_desktop_note', [
        'label' => __('桌面副文字', 'duola-pocket'),
        'section' => 'duola_desktop',
        'type' => 'text',
    ]);
    $customize->add_setting('duola_desktop_image_position', [
        'default' => 'center center',
        'sanitize_callback' => 'duola_pocket_sanitize_image_position',
    ]);
    $customize->add_control('duola_desktop_image_position', [
        'label' => __('图片焦点', 'duola-pocket'),
        'section' => 'duola_desktop',
        'type' => 'select',
        'choices' => [
            'center center' => __('居中', 'duola-pocket'),
            'center top' => __('顶部', 'duola-pocket'),
            'center bottom' => __('底部', 'duola-pocket'),
            'left center' => __('偏左', 'duola-pocket'),
            'right center' => __('偏右', 'duola-pocket'),
        ],
    ]);
}
