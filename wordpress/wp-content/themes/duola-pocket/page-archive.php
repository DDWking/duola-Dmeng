...
$years = [];
foreach (array_merge($albums, $posts) as $item) {
    $years[get_the_date('Y', $item)] = true;
}
?>
...
<section class="section year-section">
    ...
    <div class="album-grid">
        <?php foreach ($albums as $album) : ?>
            <?php if (get_the_date('Y', $album) !== $year) { continue; } ?>
            <?php setup_postdata($album); get_template_part('template-parts/album', 'card'); ?>
        <?php endforeach; wp_reset_postdata(); ?>
    </div>
    <div class="section-heading" style="margin-top: 2.5rem;"><h3>文章</h3></div>
    <div class="post-list">
        <?php foreach ($posts as $post) : ?>
            <?php if (get_the_date('Y', $post) !== $year) { continue; } ?>
            <?php setup_postdata($post); get_template_part('template-parts/post', 'row'); ?>
        <?php endforeach; wp_reset_postdata(); ?>
    </div>
</section>
<?php endforeach; ?>
